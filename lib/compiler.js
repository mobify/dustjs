...
(function(root, factory) {
  ...
  compiler.nodes = {
    ...
    // 'include': function(context, node) {
    //   var template = node[1];
    //   var partialCtx = node[2];
    //   var argsCtx = node[2];
    //   var params = node[3];
    //   var bodies = node[4];

    //   console.log('\ncompiler.include argsCtx:\n', argsCtx);

    //   // By default (no explicit context passed), empty the context.
    //   if (partialCtx.length < 2) {
    //     partialCtx.push(["literal", {}]);
    //   }

    //   return '.include(' +
    //       compiler.compileNode(context, template) +
    //       ',' + compiler.compileNode(context, partialCtx) +
    //       ',' + compiler.compileNode(context, argsCtx) +
    //       ',' + compiler.compileNode(context, bodies) +
    //       ',' + compiler.compileNode(context, params) + ')';
    // },
    ...
    'include': function(context, node) {
      ...
      var params = node[3];
      var bodies = node[4];
      var hasExplicitContext = ctx.length > 1;

      return '.include(' +
          compiler.compileNode(context, template) +
          ',' + compiler.compileNode(context, ctx) +
          ',' + compiler.compileNode(context, bodies) +
          ',' + compiler.compileNode(context, params) +
          ',' + hasExplicitContext +')';
    },
