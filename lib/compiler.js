...
(function(root, factory) {
  ...
  compiler.nodes = {
    ...
    'include': function(context, node) {
      return '.include(' +
          compiler.compileNode(context, node[1]) +        // template
          ',' + compiler.compileNode(context, node[2]) +  // context
          ',' + compiler.compileNode(context, node[4]) +  // bodies
          ',' + compiler.compileNode(context, node[3]) +  // params
          ',' + (node[2].length > 1) +                    // explicit context?
          ')';
    },
