...
(function(root) {
  ...
  // Bind polyfill. Used for include since in that case `that = this` is
  // insufficient.
  ...
  Chunk.prototype.partial = function(elem, context, params) {
    ...
    // Put the params context second to match what section does. {.} matches
    // the current context without parameters.
  ...
  Chunk.prototype.include = function(elem, context, bodies, params, hasExplicitContext) {
    ...
    // Otherwise, the context available to the included template is limited
    // to the params and bodies passed in.
    if (hasExplicitContext) {
      thisContext.blocks = context.blocks;

      if (context.stack && context.stack.tail) {
        // Grab the stack(tail) off of the previous context if we have it.
        thisContext.stack = context.stack.tail;
      }
      // Reattach the head.
      thisContext = thisContext.push(context.stack.head);
    }

    if (typeof elem === 'string') {
      thisContext.templateName = elem;
    }

    // Handle params and bodies
    // We evaluate these using the original context available to the include,
    // even when the template itself is prevented from reaching outside (which
    // is the default). To accomplish this, we need to do some extra work.
    [bodies, params].forEach(function(obj) {
      for (var prop in obj) {
        var ret;
    ...
    if (typeof elem === 'function') {
      thisChunk = this.capture(elem, thisContext, function(name, chunk) {
        thisContext.templateName = thisContext.templateName || name;
        dust.load(name, chunk, thisContext).end();
      });
    } else {
      thisChunk = dust.load(elem, this, thisContext);
    }
